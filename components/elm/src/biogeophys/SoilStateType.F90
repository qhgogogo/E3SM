  ...
  subroutine InitCold(this, bounds)
   ...
   call init_vegrootfr(bounds, nlevsoi, nlevgrnd, &
    ...
    do j = 1, nlevsoifl
       ...
       zsoifl(j) = j*1.0_r8-0.5_r8    !Han Qiu for test
    enddo
	     ...
	     nlevbed = col_pp%nlevbed(c)
             if ( more_vertlayers )then ! duplicate clay and sand values from last soil layer
                ...
                if (lev <= nlevsoi) then ! duplicate clay and sand values from 10th soil layer
                   ...
                   !om_frac = 0._r8
                   om_frac = (organic3d(g,ti,nlevsoi)/organic_max)**2._r8  !Han Qiu
                endif
             end if
             ...
             if (lun_pp%itype(l) == istdlak) then
                ...
                !om_watsat         = max(0.93_r8 - 0.1_r8   *(5.0_r8/zsapric), 0.83_r8)   !qiu added
                !om_b              = min(2.7_r8  + 9.3_r8   *(5.0_r8/zsapric), 12.0_r8)
                !om_sucsat         = min(10.3_r8 - 0.2_r8   *(5.0_r8/zsapric), 10.1_r8)
                !om_hksat          = max(0.28_r8 - 0.2799_r8*(5.0_r8/zsapric), 0.0001_r8)
